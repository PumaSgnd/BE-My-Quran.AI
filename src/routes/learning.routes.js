 ...
 *         name: category
 *         required: true
 *         description: Kategori topik (mis. "pelajaran")
 *         schema:
 *           type: string
 *           example: "pelajaran"
 *     responses:
...
/**
 * @openapi
 * /learning/topics/{topicSlug}/lessons:
 *   get:
 *     tags: [Learning]
 ...
 *       
 *       - Jika `topicSlug = "all"` → mode khusus **Quranic Surahs**:
 *         bergabung ke tabel `surahs` dan menampilkan info surah.
 ...
 *         name: topicSlug
 *         required: true
 *         description: Slug topik. Contoh: `"all"` untuk Quranic Surahs, `"hajj"` untuk topik Hajj.
 *         schema:
 *           type: string
 *           example: "all"
 *     responses:
...
/**
 * @openapi
 * /learning/lessons/{lessonId}/steps:
 *   get:
 *     tags: [Learning]
 *     summary: Semua "step/slide" dalam sebuah Pelajaran
 *     description: |
 *       Mengembalikan semua baris dari `lesson_steps` untuk `lessonId` tertentu (hanya yang aktif), urut `step_order` ASC.
 *       Struktur kolom **mengikuti tabel** (controller melakukan `SELECT *`), jadi bisa berbeda-beda. 
 *       Contoh umum kolom: `id`, `lesson_id`, `step_order`, `title`, `content_md`, `media_url`, `type`, `is_active`.
 *     parameters:
 *       - in: path
 *         name: lessonId
 *         required: true
